...
class Agent:
    ...
    def _persist_large_result(self, tool_name: str, result: str) -> str:
        ...
        if len(result.encode()) <= THRESHOLD:
            return result
        d = Path.home() / ".codeforge" / "tool-results"
    ...
    async def _execute_tool_call(self, name: str, inp: dict) -> str:
        ...
        self.audit.record("tool_started", tool=name, input=inp)
        ...
        self.audit.record(
            "tool_finished", tool=name, duration_seconds=round(time.monotonic() - started, 3),
            ok=not result.startswith(("Error", "Unknown")), result_preview=result[:500],
        )
        return result
