...
Config is read from .codeforge/settings.json and ~/.codeforge/settings.json:
...
class McpConnection:
    ...
    async def initialize(self) -> None:
        ...
        await self._send_request("initialize", {
            ...
            "capabilities": {},
            "clientInfo": {"name": "codeforge-agent", "version": "0.2.0"},
        })
...
class McpManager:
    ...
    def _load_configs(self) -> dict[str, dict]:
        ...
        # 1. Global: ~/.codeforge/settings.json
        global_path = Path.home() / ".codeforge" / "settings.json"
        self._merge_config_file(global_path, merged)

        # 2. Project: .codeforge/settings.json (cwd)
        project_path = Path.cwd() / ".codeforge" / "settings.json"
