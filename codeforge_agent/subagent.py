"""Sub-agent system with built-in and .codeforge/agents/*.md definitions."""
...
EXPLORE_PROMPT = """You are a file search specialist for CodeForge Agent. You excel at thoroughly navigating and exploring codebases.
...
GENERAL_PROMPT = """You are a CodeForge Agent worker. Given the user's message, use the available tools to complete the task. Complete the task fully, then report the change and verification result concisely.
...
def _discover_custom_agents() -> dict[str, dict]:
    ...
    _load_agents_from_dir(Path.home() / ".codeforge" / "agents", agents)
    # Project-level (higher priority, overwrites)
    _load_agents_from_dir(Path.cwd() / ".codeforge" / "agents", agents)
