"""Tool definitions and execution with permission checks and edit freshness guards."""
...
tool_definitions: list[ToolDef] = [
    ...
    {
        "name": "skill",
        "description": "Invoke a registered skill by name. Skills are prompt templates loaded from .codeforge/skills/. Returns the resolved prompt to follow.",
        "input_schema": {
            "type": "object",
            "properties": {
...
def _web_fetch(inp: dict) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "codeforge-agent/0.2"})
    try:
...
# ─── Permission rules (.codeforge/settings.json) ─────────────
...
def load_permission_rules() -> dict:
    ...
    user_settings = _load_settings(Path.home() / ".codeforge" / "settings.json")
    project_settings = _load_settings(Path.cwd() / ".codeforge" / "settings.json")
